...
export default function Home() {
  ...
  const [tab, setTab] = useState<"overview" | "transactions" | "insights" | "intelligence" | "settings" | "save" | "goals" | "pix">("overview");
  ...
  return <main className="app-shell"><aside className="sidebar"><div className="brand"><span>f</span> fluxo</div><nav><button className={tab === "overview" ? "active" : ""} onClick={() => { setSelectedMonth(months.at(-1) ?? ""); setSelectedCategory(""); setSelectedMerchant(""); setSelectedSource(""); setTab("overview"); }}>⌂ <span>Visão geral</span></button><button className={tab === "transactions" ? "active" : ""} onClick={() => { setSelectedCategory(""); setSelectedMerchant(""); setSelectedSource(""); setTab("transactions"); }}>↕ <span>Transações</span></button><button className={tab === "transactions" && selectedSource === "invoice" ? "active" : ""} onClick={() => { setSelectedCategory(""); setSelectedMerchant(""); setSelectedSource("invoice"); setTab("transactions"); }}>▣ <span>Cartão de crédito</span></button><button className={tab === "transactions" && selectedSource === "statement" ? "active" : ""} onClick={() => { setSelectedCategory(""); setSelectedMerchant(""); setSelectedSource("statement"); setTab("transactions"); }}>⌂ <span>Extrato bancário</span></button><button className={tab === "pix" ? "active" : ""} onClick={() => setTab("pix")}>↗ <span>PIX</span></button><button className={tab === "insights" ? "active" : ""} onClick={() => setTab("insights")}>✦ <span>Comparar</span><b>{months.length}</b></button><button className={tab === "intelligence" ? "active" : ""} onClick={() => setTab("intelligence")}>◉ <span>Análise inteligente</span></button><button className={tab === "settings" ? "active" : ""} onClick={() => setTab("settings")}>⚙ <span>Configurações</span></button><button className={tab === "save" ? "active" : ""} onClick={() => setTab("save")}>♧ <span>Economizar</span></button><button className={tab === "goals" ? "active" : ""} onClick={() => setTab("goals")}>◎ <span>Objetivos</span></button></nav><button className="new-import" onClick={() => setView("upload")}>＋ Importar histórico</button></aside><section className="content"><header><div>{tab !== "overview" && <button className="back-button back-overview" onClick={() => { setSelectedMonth(months.at(-1) ?? ""); setSelectedCategory(""); setSelectedMerchant(""); setSelectedSource(""); setTab("overview"); }}>← Voltar à visão geral</button>}<p className="eyebrow">HISTÓRICO FINANCEIRO</p><h2>{tab === "overview" ? "Visão geral" : tab === "transactions" ? "Transações" : tab === "insights" ? "Comparação financeira" : tab === "intelligence" ? "Análise inteligente" : tab === "settings" ? "Configurações" : tab === "save" ? "Plano para economizar" : tab === "goals" ? "Objetivos financeiros" : "PIX"}</h2></div><div className="header-tools"><select value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)}><option value="">Todos os meses</option>{months.map((month) => <option key={month} value={month}>{monthLabel(month)}</option>)}</select><div className="avatar">{user?.displayName?.slice(0, 2).toUpperCase() ?? "BR"}</div></div></header>
...
function PixPanel({ transactions, selectedMonth }: { transactions: ParsedTransaction[]; selectedMonth: string }) {
  const pix = transactions.filter((item) => (!selectedMonth || (item.month ?? item.date.slice(0,7)) === selectedMonth) && /pix (?:enviado|recebido)/i.test(item.description));
...
function SmartInsights({ transactions, expenses, months, selectedMonth, invoiceTotal, monthlyGoal, cardLimit, setMonthlyGoal, setCardLimit, budgets, onShowMerchant }: { transactions: ParsedTransaction[]; expenses: ParsedTransaction[]; months: string[]; selectedMonth: string; invoiceTotal: number; monthlyGoal: number; cardLimit: number; setMonthlyGoal: (value: number) => void; setCardLimit: (value: number) => void; budgets: Partial<Record<Category, number>>; onShowMerchant: (merchant: string) => void }) {
