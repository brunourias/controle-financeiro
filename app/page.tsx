...
export default function Home() {
  ...
  const distributionTotal = visibleTransactions.reduce((sum, item) => sum + Math.abs(item.amount), 0);
  const categoryData = useMemo(() => CATEGORY_NAMES.map((name) => { const value = visibleTransactions.filter((item) => item.category === name).reduce((sum, item) => sum + Math.abs(item.amount), 0); return { name, value, percentage: distributionTotal ? Math.round(value / distributionTotal * 100) : 0, color: CATEGORY_META[name].color }; }).filter((item) => item.value > 0).sort((a, b) => b.value - a.value), [visibleTransactions, distributionTotal]);
