...
export default function Home() {
  ...
  const expenses = useMemo(() => allOutflows.filter((item) => !MOVEMENT_CATEGORIES.includes(item.category)), [allOutflows]);
  ...
  const monthlySeries = useMemo(() => months.map((month) => {
    const expensesForMonth = transactions.filter((item) => (item.month ?? item.date.slice(0, 7)) === month && item.amount < 0).map((item) => CATEGORY_NAMES.includes(item.category) ? item : { ...item, category: "Outros" as Category }).filter((item) => !MOVEMENT_CATEGORIES.includes(item.category));
  ...
  return <main className="app-shell"><aside className="sidebar"><div className="brand"><span>f</span> fluxo</div><nav><button className={tab === "overview" ? "active" : ""} onClick={() => { setSelectedMonth(months.at(-1) ?? ""); setSelectedCategory(""); setSelectedMerchant(""); setSelectedGroup(""); setSelectedSource(""); setTab("overview"); }}>⌂ <span>Visão geral</span></button><button className={tab === "transactions" ? "active" : ""} onClick={() => { setSelectedCategory(""); setSelectedMerchant(""); setSelectedGroup(""); setSelectedSource(""); setTab("transactions"); }}>↕ <span>Transações</span></button><button className={tab === "transactions" && selectedSource === "invoice" ? "active" : ""} onClick={() => { setSelectedCategory(""); setSelectedMerchant(""); setSelectedGroup(""); setSelectedSource("invoice"); setTab("transactions"); }}>▣ <span>Cartão de crédito</span></button><button className={tab === "transactions" && selectedSource === "statement" ? "active" : ""} onClick={() => { setSelectedCategory(""); setSelectedMerchant(""); setSelectedGroup(""); setSelectedSource("statement"); setTab("transactions"); }}>⌂ <span>Extrato bancário</span></button><button className={tab === "pix" ? "active" : ""} onClick={() => setTab("pix")}>↗ <span>PIX</span></button><button className={tab === "income" ? "active" : ""} onClick={() => { setSelectedCategory(""); setSelectedMerchant(""); setSelectedGroup(""); setSelectedSource(""); setTab("income"); }}>↓ <span>Entradas na conta</span></button><button className={tab === "insights" ? "active" : ""} onClick={() => setTab("insights")}>✦ <span>Comparar</span></button><button className={tab === "intelligence" ? "active" : ""} onClick={() => setTab("intelligence")}>◉ <span>Análise inteligente</span></button><button className={tab === "assistant" ? "active" : ""} onClick={() => setTab("assistant")}>✦ <span>Assistente IA</span></button><button className={tab === "settings" ? "active" : ""} onClick={() => setTab("settings")}>⚙ <span>Configurações</span></button><button className={tab === "save" ? "active" : ""} onClick={() => setTab("save")}>♧ <span>Economizar</span></button><button className={tab === "goals" ? "active" : ""} onClick={() => setTab("goals")}>◎ <span>Objetivos</span></button></nav><button className="new-import" onClick={() => setView("upload")}>＋ Importar histórico</button></aside><section className="content"><header><div>{tab !== "overview" && <button className="back-button back-overview" onClick={() => { setSelectedMonth(months.at(-1) ?? ""); setSelectedCategory(""); setSelectedMerchant(""); setSelectedSource(""); setTab("overview"); }}>← Voltar à visão geral</button>}<p className="eyebrow">HISTÓRICO FINANCEIRO</p><h2>{tab === "overview" ? "Visão geral" : tab === "transactions" ? "Transações" : tab === "insights" ? "Comparação financeira" : tab === "intelligence" ? "Análise inteligente" : tab === "assistant" ? "Assistente financeiro" : tab === "settings" ? "Configurações" : tab === "income" ? "Entradas na conta" : tab === "save" ? "Plano para economizar" : tab === "goals" ? "Objetivos financeiros" : "PIX"}</h2></div><div className="header-tools"><select value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)}><option value="">Todos os meses</option>{months.map((month) => <option key={month} value={month}>{monthLabel(month)}</option>)}</select><div className="avatar">{user?.displayName?.slice(0, 2).toUpperCase() ?? "BR"}</div></div></header>
    ...
    {tab === "overview" && <><div className="summary-grid"><article className="hero-card"><p>Gastos de consumo em {monthLabel(selectedMonth)}</p><strong>{money(total)}</strong><span>{expenses.length} lançamento(s) de consumo: fatura e débitos da conta</span><div className="spark"><i/><i/><i/><i/><i/><i/><i/><i/></div></article><article><p>Cartão de crédito</p><strong>{money(invoiceTotal)}</strong><span>{expenses.filter((item) => item.source === "invoice").length} compra(s) na fatura</span><div className="progress"><i style={{ width: (total ? invoiceTotal / total * 100 : 0) + "%" }} /></div></article><article><p>Conta corrente</p><strong>{money(statementTotal)}</strong><span>boletos, débitos e despesas no extrato</span><div className="progress green"><i style={{ width: (total ? statementTotal / total * 100 : 0) + "%" }} /></div></article></div>{movementTotal > 0 && <div className="movement-note"><span>↔</span><p><strong>{money(movementTotal)} em PIX e transferências</strong> foram mantidos em Movimentações e PIX; não entram em “Onde você mais gastou”.</p><button onClick={() => setTab("pix")}>Ver PIX</button></div>}<div className="dashboard-grid"><article className="category-card"><div className="card-title"><div><p className="eyebrow">GASTOS DE CONSUMO</p><h3>Onde você mais gastou</h3></div></div><div className="category-body"><div className="donut" style={{ background: donut }}><span><b>{money(total)}</b><small>total</small></span></div><div className="legend">{categoryData.map((item) => <button key={item.name} className="category-filter" onClick={() => { setSelectedCategory(item.name); setSelectedGroup(""); setTab("transactions"); }}><i style={{ background: item.color }} /><span>{item.name}</span><strong>{money(item.value)}</strong><b>{item.percentage}%</b></button>)}</div></div></article><article className="saving-card"><span className="bulb">✦</span><p className="eyebrow">PRIORIDADE DO MÊS</p><h3>{categoryData[0]?.name ?? "Sem gastos"}: <em>{money(categoryData[0]?.value ?? 0)}</em></h3><p>{categoryData[0] ? <>Representa {categoryData[0].percentage}% dos gastos de consumo.</> : <>Importe dados para ver sua principal categoria.</>}</p><button onClick={() => setTab("intelligence")}>Ver análise →</button></article></div><TransactionList items={expenses.slice(0, 5)} title="Últimos gastos de consumo" action={() => setTab("transactions")} /></>}
...
function FinancialAssistant({ transactions, months, selectedMonth, goalTarget, goalSaved, onShowCategory, onShowFlexible }: { transactions: ParsedTransaction[]; months: string[]; selectedMonth: string; goalTarget: number; goalSaved: number; onShowCategory: (category: Category) => void; onShowFlexible: () => void }) {
  ...
  const expenses = current.filter((item) => item.amount < 0);
  ...
  const averageOutflow = months.length ? months.reduce((sum, month) => sum + Math.abs(transactions.filter((item) => (item.month ?? item.date.slice(0, 7)) === month && item.amount < 0).reduce((partial, item) => partial + item.amount, 0)), 0) / months.length : 0;
