...
export default function Home() {
  ...
  const [tab, setTab] = useState<"overview" | "transactions" | "insights" | "intelligence" | "save" | "goals" | "pix">("overview");
  ...
  return <main className="app-shell"><aside className="sidebar"><div className="brand"><span>f</span> fluxo</div><nav><button className={tab === "overview" ? "active" : ""} onClick={() => setTab("overview")}>⌂ <span>Visão geral</span></button><button className={tab === "transactions" ? "active" : ""} onClick={() => { setSelectedCategory(""); setSelectedMerchant(""); setSelectedSource(""); setTab("transactions"); }}>↕ <span>Transações</span></button><button className={tab === "transactions" && selectedSource === "invoice" ? "active" : ""} onClick={() => { setSelectedCategory(""); setSelectedMerchant(""); setSelectedSource("invoice"); setTab("transactions"); }}>▣ <span>Cartão de crédito</span></button><button className={tab === "transactions" && selectedSource === "statement" ? "active" : ""} onClick={() => { setSelectedCategory(""); setSelectedMerchant(""); setSelectedSource("statement"); setTab("transactions"); }}>⌂ <span>Extrato bancário</span></button><button className={tab === "pix" ? "active" : ""} onClick={() => setTab("pix")}>↗ <span>PIX</span></button><button className={tab === "insights" ? "active" : ""} onClick={() => setTab("insights")}>✦ <span>Comparar</span><b>{months.length}</b></button><button className={tab === "intelligence" ? "active" : ""} onClick={() => setTab("intelligence")}>◉ <span>Análise inteligente</span></button><button className={tab === "save" ? "active" : ""} onClick={() => setTab("save")}>♧ <span>Economizar</span></button><button className={tab === "goals" ? "active" : ""} onClick={() => setTab("goals")}>◎ <span>Objetivos</span></button></nav><button className="new-import" onClick={() => setView("upload")}>＋ Importar histórico</button></aside><section className="content"><header><div>{tab !== "overview" && <button className="back-button" onClick={() => setTab("overview")}>← Voltar à visão geral</button>}<p className="eyebrow">HISTÓRICO FINANCEIRO</p><h2>{tab === "overview" ? "Visão geral" : tab === "transactions" ? "Transações" : tab === "insights" ? "Comparação financeira" : tab === "intelligence" ? "Análise inteligente" : tab === "save" ? "Plano para economizar" : tab === "goals" ? "Objetivos financeiros" : "PIX"}</h2></div><div className="header-tools"><select value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)}><option value="">Todos os meses</option>{months.map((month) => <option key={month} value={month}>{monthLabel(month)}</option>)}</select><div className="avatar">{user?.displayName?.slice(0, 2).toUpperCase() ?? "BR"}</div></div></header>
    ...
    {tab === "insights" && <><MonthlyEvolution series={monthlySeries} /><BudgetAndRules expenses={expenses} rules={classificationRules} budgets={categoryBudgets} setBudgets={setCategoryBudgets} setRules={setClassificationRules} /></>}
