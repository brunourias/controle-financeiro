...
export default function Home() {
  ...
  const [tab, setTab] = useState<"overview" | "transactions" | "insights" | "save">("overview");
  ...
  return <main className="app-shell"><aside className="sidebar"><div className="brand"><span>f</span> fluxo</div><nav><button className={tab === "overview" ? "active" : ""} onClick={() => setTab("overview")}>⌂ <span>Visão geral</span></button><button className={tab === "transactions" ? "active" : ""} onClick={() => setTab("transactions")}>↕ <span>Transações</span></button><button className={tab === "insights" ? "active" : ""} onClick={() => setTab("insights")}>✦ <span>Comparar</span><b>{months.length}</b></button><button className={tab === "save" ? "active" : ""} onClick={() => setTab("save")}>♧ <span>Economizar</span></button></nav><button className="new-import" onClick={() => setView("upload")}>＋ Importar histórico</button></aside><section className="content"><header><div><p className="eyebrow">HISTÓRICO FINANCEIRO</p><h2>{tab === "overview" ? "Visão geral" : tab === "transactions" ? "Transações" : tab === "insights" ? "Comparação mensal" : "Plano para economizar"}</h2></div><div className="header-tools"><select value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)}><option value="">Todos os meses</option>{months.map((month) => <option key={month} value={month}>{monthLabel(month)}</option>)}</select><div className="avatar">{user?.displayName?.slice(0, 2).toUpperCase() ?? "BR"}</div></div></header>
...
function SavingsCoach({ expenses, transactions, selectedMonth, goalTarget, goalSaved, goalDate, monthlyIncome, detectedIncome, setGoalTarget, setGoalSaved, setGoalDate, setMonthlyIncome }: { expenses: ParsedTransaction[]; transactions: ParsedTransaction[]; selectedMonth: string; goalTarget: number; goalSaved: number; goalDate: string; monthlyIncome: number; detectedIncome: number; setGoalTarget: (value: number) => void; setGoalSaved: (value: number) => void; setGoalDate: (value: string) => void; setMonthlyIncome: (value: number) => void }) {
