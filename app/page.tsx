...
export default function Home() {
  ...
  if (view === "upload") return <UploadView files={files} ready={ready} parsing={parsing} error={error} user={user} cloudDocuments={cloudDocuments} onFiles={onFiles} analyze={analyze} startDemo={startDemo} setError={setError} backToDashboard={transactions.length ? () => setView("dashboard") : undefined} />;
...
function UploadView({ files, ready, parsing, error, user, cloudDocuments, onFiles, analyze, startDemo, setError, backToDashboard }: { files: Record<FileKind, File[]>; ready: boolean; parsing: boolean; error: string; user: AppUser | null; cloudDocuments: CloudDocument[]; onFiles: (kind: FileKind, files: File[]) => void; analyze: () => void; startDemo: () => void; setError: (value: string) => void; backToDashboard?: () => void }) {
